...
namespace Formulario_soporte.Models
{
    ...
    // <<< INICIO: ACTUALIZAR EL [Bind] >>>
    [Bind("IdReporte,IdUsuarioTecnico,IdEquipo,IdSucursal,FechaReporte," +
          // Mantenimiento Lógico
          "Defragmentar,WindowsConfiguracion,ActualizacionDrivers," +
          // Mantenimiento Físico
          "LimpiezaTotal,DiscoDuroFisico,TecladoFisico,MemoriaFisico,RedFisico,PantallaMonitorFisico," +
          // Mantenimiento Reemplazo
          "DiscoDuroReemplazo,TecladoReemplazo,MemoriaReemplazo,RedReemplazo,PantallaMonitorReemplazo," +
          // Resto de los campos
          "Descripcion,TrabajoRealizado,Estado,IdUsuarioResponsable")] Reporte reporte,
    // <<< FIN: ACTUALIZAR EL [Bind] >>>
    string SignatureData)
        ...
        public async Task<IActionResult> Edit(int id) // 👈 Solo necesitamos el 'id'
        {
            ...
            // <<< INICIO: ACTUALIZAR PROPIEDADES EN TryUpdateModelAsync >>>
            var isUpdateSuccessful = await TryUpdateModelAsync(
                reporteExistente,
                "",
                r => r.IdUsuarioTecnico, r => r.IdEquipo, r => r.IdSucursal, r => r.FechaReporte,
                // Mantenimiento Lógico
                r => r.Defragmentar, r => r.WindowsConfiguracion, r => r.ActualizacionDrivers,
                // Mantenimiento Físico
                r => r.LimpiezaTotal, r => r.DiscoDuroFisico, r => r.TecladoFisico, r => r.MemoriaFisico,
                r => r.RedFisico, r => r.PantallaMonitorFisico,
                // Mantenimiento Reemplazo
                r => r.DiscoDuroReemplazo, r => r.TecladoReemplazo, r => r.MemoriaReemplazo,
                r => r.RedReemplazo, r => r.PantallaMonitorReemplazo,
                // Resto de los campos
                r => r.Descripcion, r => r.TrabajoRealizado, r => r.Estado, r => r.IdUsuarioResponsable
            );
            // <<< FIN: ACTUALIZAR PROPIEDADES EN TryUpdateModelAsync >>>
